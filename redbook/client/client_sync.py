...
import requests
from playwright.sync_api import BrowserContext, Cookie, Page, sync_playwright
...
class GetXS:
    ...
    def get_header(self, api, data=''):
        if not self.client:
            self.client = get_client(self.cookies)
        xs = (self.client._pre_headers(api, data)) if api else {}
        return self.client.headers | xs


def get_client(cookies=None):
...
class XiaoHongShuClient:
    ...
    def _pre_headers(self, api: str, data=None) -> dict:
        encrypt_params = self.playwright_page.evaluate(
    ...
    def get(self, url: str, api=''):
        xs = (self._pre_headers(api)) if api else {}
        return requests.get(url=f"{url}{api}", headers=self.headers | xs)

    def post(self, url: str, api: str, data: dict):
        headers = self._pre_headers(api, data)
